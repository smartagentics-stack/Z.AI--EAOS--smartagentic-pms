...
export function generateReports(
  ...
  try {
    ...
    // Not a git repo or git not installed — use 'unknown'
  }
  try {
    branch = execSync('git branch --show-current', { cwd: repoRoot, encoding: 'utf-8' }).trim();
  } catch {
    // Not a git repo or git not installed — use 'unknown'
  }
  ...
  writeFileSync(
    resolve(evidenceDir, 'verification-summary.json'),
  ...
  if (governanceResult) {
    writeFileSync(
      resolve(evidenceDir, 'governance-report.json'),
  ...
  const archResults = results.filter(
    (r) => r.name === 'architecture-drift' || r.name === 'serialization-consistency',
  );
  writeFileSync(
    resolve(evidenceDir, 'architecture-report.json'),
    JSON.stringify(archResults, null, 2),
  );

  // Write security report
  const securityResults = results.filter(
    (r) => r.name === 'dependency-audit' || r.name === 'forbidden-code',
  );
  writeFileSync(
    resolve(evidenceDir, 'security-report.json'),

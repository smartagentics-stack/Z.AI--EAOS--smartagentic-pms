  ...
  return {
    ...
    async write() {
      const record: SyncRecord = {
        id: randomUUID(),
        idempotencyKey: `${clientId}-${++sequence}`,
        payload: {
          name: `r-${sequence}`,
      ...
      if (result.acked) {
        metrics.acksReceived[clientId]++;
        metrics.latencies.push(result.latencyMs);
      } else {
        metrics.acksMissed[clientId]++;
      }
    },
  };
}
...
async function main() {
  ...
  const clientA = createClient('A', join(tmpdir(), 'spike-01-client-a.db'), 17001, 17002);
  const clientB = createClient('B', join(tmpdir(), 'spike-01-client-b.db'), 17002, 17001);
  ...
  const wA = setInterval(
    () => clientA.write().catch((e) => metrics.errors.push(`A:${e.message}`)),
    WRITE_INTERVAL_MS,
  );
  const wB = setInterval(
    () => clientB.write().catch((e) => metrics.errors.push(`B:${e.message}`)),
    WRITE_INTERVAL_MS,
  );
  ...
  const progI = setInterval(() => {
    ...
    process.stdout.write(
      `\rT+${e}s w:A=${metrics.recordsWritten.A} B=${metrics.recordsWritten.B} miss:A=${metrics.acksMissed.A} B=${metrics.acksMissed.B} int:${metrics.networkInterruptions} st:A=${clientA.client.getState()} B=${clientB.client.getState()}`,
    );
  }, 5000);
  ...
  const p50 = l[Math.floor(l.length * 0.5)] || 0,
  ...
  writeFileSync(
    `${import.meta.dirname}/../results.json`,
    JSON.stringify(
      {
        duration_seconds: DURATION_SECONDS,
        records: {
          clientA: { written: metrics.recordsWritten.A, inDb: recordsA.length, duplicates: dupA },
          clientB: { written: metrics.recordsWritten.B, inDb: recordsB.length, duplicates: dupB },
        },
        sync: {
          totalUnique: new Set([...keysA, ...keysB]).size,
          onlyInA,
          onlyInB,
  ...
  const checks = [
    ...
    {
      n: 'S4: Zero data loss',
      p: onlyInA.length + onlyInB.length === 0,
      v: `${onlyInA.length + onlyInB.length}`,
    },
    {
      n: 'S5: Zero loss (interruption)',
      p: onlyInA.length + onlyInB.length === 0,
      v: `${onlyInA.length + onlyInB.length}`,
    },
